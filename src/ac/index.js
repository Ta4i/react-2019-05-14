...
export const loadDishes = id => (dispatch, getState) => {
  ...
  if (!state.dishes.loaded[id] && !state.dishes.loading[id]) {
    dispatch({ type: LOAD_DISHES + START, restaurantId: id });
    fetch(`http://localhost:3001/api/dishes?id=${id}`)
      ...
      .catch(e => {
        dispatch({ type: LOAD_DISHES + FAIL, error: e, restaurantId: id });
      });
  }
};
